#!/usr/bin/env python3
"""
Duke Energy Utility Bill Fetcher

Main entry point for fetching and parsing Duke Energy utility bills.
...
def parse_existing_pdfs(pdf_dir: Path) -> list[EnergyBillData]:
    """Parse all Duke Energy PDFs in a directory."""
    bills = []
    ...
    # Look for Duke Energy specific PDFs or all PDFs
    pdf_files = list(pdf_dir.glob("*duke*.pdf")) + list(pdf_dir.glob("*energy*.pdf"))
    if not pdf_files:
        # Fall back to all PDFs in duke-energy subfolder
...
def test_parser():
    ...
    # Look for duke energy sample
    sample_files = list(sample_dir.glob("*duke*.pdf"))
    if not sample_files:
        print("No Duke Energy sample files found")
        return False
...
def fetch_from_portal(headless: bool = True, account_filter: str = None) -> FetchResult:
    """Fetch bills from the Duke Energy portal."""
    ...
    if not username or not password:
        print("Error: DUKE_ELECTRIC_USER and DUKE_ELECTRIC_PASS must be set")
        return FetchResult(success=False, errors=["Missing credentials"])

    download_dir = get_project_root() / "data" / "downloaded-bills" / "duke-energy"
    ...
    return scraper.fetch_bills(headless=headless, account_filter=account_filter)
...
def main():
    parser = argparse.ArgumentParser(
        description="Duke Energy Utility Bill Fetcher",
    ...
    parser.add_argument(
        "--account",
        type=str,
        help="Specific account number to switch to (for multi-account)",
    )
    ...
    if args.test:
        ...
        if sample_dir.exists():
            # Parse duke energy samples from sample-bills
            for pdf in sample_dir.glob("*duke*.pdf"):
                try:
        ...
        # Fetch from portal
        print("Fetching bills from Duke Energy portal...")
        result = fetch_from_portal(headless=not args.visible, account_filter=args.account)

#!/usr/bin/env python3
"""
SMUD (Sacramento Municipal Utility District) Bill Fetcher

Main entry point for fetching and parsing SMUD electric utility bills.
...
from models import SmudBillData, DocumentType, FetchResult
...
def parse_existing_pdfs(pdf_dir: Path) -> list[SmudBillData]:
...
def fetch_from_portal(headless: bool = True) -> FetchResult:
    """Fetch bills from the SMUD portal."""
    from scraper import SmudScraper
...
def output_json(bills: list[SmudBillData], output_path: Path = None):
...
def main():
    parser = argparse.ArgumentParser(
        description="SMUD Electric Bill Fetcher",
    ...
    if args.test:
        ...
        # Fetch from portal
        print("Fetching bills from SMUD portal...")

#!/usr/bin/env python3
"""
Durham Water Utility Bill Fetcher

Main entry point for fetching and parsing Durham water utility bills.
...
from models import WaterBillData, DocumentType, FetchResult
...
def parse_existing_pdfs(pdf_dir: Path) -> list[WaterBillData]:
...
def test_parser():
    ...
    bills = parse_existing_pdfs(sample_dir)
    ...
    for bill in bills:
        print("\n" + "-"*40)
        print(json.dumps(bill.to_dict(), indent=2, default=str))

    return len(bills) > 0


def fetch_from_portal(headless: bool = True) -> FetchResult:
    """Fetch bills from the Durham Water portal."""
    from scraper import DurhamWaterScraper
...
def output_json(bills: list[WaterBillData], output_path: Path = None):
...
def main():
    parser = argparse.ArgumentParser(
        description="Durham Water Utility Bill Fetcher",
    ...
    if args.test:
        ...
        # Fetch from portal
        print("Fetching bills from Durham Water portal...")
        ...
        if args.json or args.output:
            ...
            if attention_bills:
                ...
                for bill in attention_bills:
                    ...
                    if bill.due_date:
                        print(f"  Due: {bill.due_date}")
                    if bill.disconnect_date:
                        print(f"  DISCONNECT DATE: {bill.disconnect_date}")

...
import { EnbridgeGasDocType } from "@/generated/prisma/client";
...
interface EnbridgeGasBill {
  ...
  gas_charges: number;
  basic_facilities_charge: number;
  taxes: number;
...
interface FetchResults {
  timestamp: string;
  count: number;
  bills: EnbridgeGasBill[];
  requires_attention: EnbridgeGasBill[];
}

// Map document type string to enum
function mapDocType(docType: string): EnbridgeGasDocType {
  switch (docType) {
    case "bill":
      return EnbridgeGasDocType.BILL;
    case "disconnect_notice":
      return EnbridgeGasDocType.DISCONNECT_NOTICE;
    default:
      return EnbridgeGasDocType.UNKNOWN;
  }
}

// Store parsed bills in the database
async function storeParsedBills(
  bills: (EnbridgeGasBill & { matched_property_id: string | null })[]
  ...
  for (const bill of bills) {
    try {
      ...
      const existing = await prisma.enbridgeGasParsedBill.findFirst({
        where: {
      ...
      if (existing) {
        await prisma.enbridgeGasParsedBill.update({
          ...
          data: {
            ...
            thermsUsed: bill.therms_used || 0,
            meterNumber: bill.meter_number,
            gasCharges: bill.gas_charges || 0,
            basicFacilitiesCharge: bill.basic_facilities_charge || 0,
            taxes: bill.taxes || 0,
        ...
        await prisma.enbridgeGasParsedBill.create({
          data: {
            accountNumber: bill.account_number,
            serviceAddress: bill.service_address,
            documentType: mapDocType(bill.document_type),
...
async function loadStoredBills() {
  const bills = await prisma.enbridgeGasParsedBill.findMany({
  ...
  return bills.map((bill) => ({
    ...
    therms_used: bill.thermsUsed,
    meter_number: bill.meterNumber,
    gas_charges: bill.gasCharges,
    basic_facilities_charge: bill.basicFacilitiesCharge,
    taxes: bill.taxes,
...
export async function GET(req: NextRequest) {
  try {
    ...
    const scriptPath = path.join(projectRoot, "scripts", "enbridge-gas", "main.py");
    const outputDir = path.join(projectRoot, "data", "enbridge-bills");
    ...
    const venvPython = path.join(projectRoot, "scripts", "enbridge-gas", ".venv", "bin", "python");

    try {
      const { stderr } = await execAsync(
        `${venvPython} ${scriptPath} ${args.join(" ")}`,
        {
          cwd: path.join(projectRoot, "scripts", "enbridge-gas"),
          timeout: 300000, // 5 minute timeout
          env: {
            ...process.env,
            PYTHONPATH: path.join(projectRoot, "scripts", "enbridge-gas"),
            PLAYWRIGHT_BROWSERS_PATH: path.join(projectRoot, "scripts", "enbridge-gas", ".cache", "ms-playwright"),
          },
        }
      );
      ...
      if (stderr.includes("playwright install") || stderr.includes("Executable doesn't exist")) {
        return NextResponse.json(
          {
            error: "Playwright browsers not installed. Run 'cd scripts/enbridge-gas && ./setup.sh' to enable portal downloads.",
            details: "The scraper requires Playwright browsers to log into the Enbridge Gas portal.",
          },
          { status: 500 }
        );
      }
      ...
      try {
        ...
        return NextResponse.json(
          {
            error: "Failed to run Enbridge Gas scraper",
    ...
    console.log(`Enbridge Gas: Stored ${stored} new bills, updated ${updated} existing bills`);
    ...
    console.error("Enbridge Gas API error:", error);
    return NextResponse.json(
      { error: "Failed to fetch Enbridge Gas bills" },
      { status: 500 }
    );
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { bills } = body as { bills: (EnbridgeGasBill & { matched_property_id?: string; stored_id?: string })[] };
    ...
    console.error("Failed to import Enbridge Gas bills:", error);
    return NextResponse.json(

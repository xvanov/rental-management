...
export async function GET(req: NextRequest) {
  try {
    ...
    const outputDir = path.join(process.cwd(), "data", "downloaded-bills", "wake-electric");
    ...
    if (!result.success) {
      return NextResponse.json(
        { error: "Failed to run Wake Electric scraper", details: result.error },
        { status: 500 }
      );
    }

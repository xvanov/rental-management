...
import { SmudDocType } from "@/generated/prisma/client";
...
interface FetchResults {
  ...
  count: number;
  bills: SmudBill[];
  requires_attention: SmudBill[];
}

// Map document type string to enum
function mapDocType(docType: string): SmudDocType {
  switch (docType) {
    case "bill":
      return SmudDocType.BILL;
    case "disconnect_notice":
      return SmudDocType.DISCONNECT_NOTICE;
    default:
      return SmudDocType.UNKNOWN;
  }
}

// Store parsed bills in the database
async function storeParsedBills(
  bills: (SmudBill & { matched_property_id: string | null })[]
  ...
  for (const bill of bills) {
    try {
      ...
      const existing = await prisma.smudParsedBill.findFirst({
        where: {
      ...
      if (existing) {
        await prisma.smudParsedBill.update({
        ...
        await prisma.smudParsedBill.create({
          data: {
            accountNumber: bill.account_number,
            serviceAddress: bill.service_address,
            documentType: mapDocType(bill.document_type),
...
async function loadStoredBills() {
  const bills = await prisma.smudParsedBill.findMany({
...
export async function GET(req: NextRequest) {
  try {
    ...
    const scriptPath = path.join(projectRoot, "scripts", "smud", "main.py");
    const outputDir = path.join(projectRoot, "data", "smud-bills");
    ...
    const venvPython = path.join(projectRoot, "scripts", "smud", ".venv", "bin", "python");

    try {
      const { stderr } = await execAsync(
        `${venvPython} ${scriptPath} ${args.join(" ")}`,
        {
          cwd: path.join(projectRoot, "scripts", "smud"),
          timeout: 300000, // 5 minute timeout
          env: {
            ...process.env,
            PYTHONPATH: path.join(projectRoot, "scripts", "smud"),
            PLAYWRIGHT_BROWSERS_PATH: path.join(projectRoot, "scripts", "smud", ".cache", "ms-playwright"),
          },
        }
      );
      ...
      if (stderr.includes("playwright install") || stderr.includes("Executable doesn't exist")) {
        return NextResponse.json(
          {
            error: "Playwright browsers not installed. Run 'playwright install' in the scripts/smud/.venv to enable portal downloads.",
            details: "The scraper requires Playwright browsers to log into the SMUD portal.",
          },
          { status: 500 }
        );
      }
      ...
      try {
        ...
        return NextResponse.json(
          {
            error: "Failed to run SMUD scraper",
    ...
    console.log(`SMUD: Stored ${stored} new bills, updated ${updated} existing bills`);
    ...
    console.error("SMUD API error:", error);
    return NextResponse.json(
      { error: "Failed to fetch SMUD bills" },
      { status: 500 }
    );
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { bills } = body as { bills: (SmudBill & { matched_property_id?: string; stored_id?: string })[] };
    ...
    console.error("Failed to import SMUD bills:", error);
    return NextResponse.json(

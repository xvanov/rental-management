...
interface FetchResults {
  timestamp: string;
  count: number;
  bills: DurhamWaterBill[];
  requires_attention: DurhamWaterBill[];
}

// Map document type string to enum
function mapDocType(docType: string): DurhamWaterDocType {
  switch (docType) {
    case "bill":
      return DurhamWaterDocType.BILL;
    case "delinquency_notice":
      return DurhamWaterDocType.DELINQUENCY_NOTICE;
    default:
      return DurhamWaterDocType.UNKNOWN;
  }
}

// Address matching functions moved to @/lib/utilities/address-matching

// Store parsed bills in the database
async function storeParsedBills(
  bills: (DurhamWaterBill & { matched_property_id: string | null })[]
  ...
  for (const bill of bills) {
    try {
      ...
      const existing = await prisma.durhamWaterParsedBill.findFirst({
        where: {
      ...
      if (existing) {
        await prisma.durhamWaterParsedBill.update({
          ...
          data: {
            ...
            balanceForward: bill.balance_forward || 0,
        ...
        await prisma.durhamWaterParsedBill.create({
          data: {
            accountNumber: bill.account_number,
            serviceLocation: bill.service_location,
            documentType: mapDocType(bill.document_type),
...
async function loadStoredBills() {
  const bills = await prisma.durhamWaterParsedBill.findMany({
  ...
  return bills.map((bill) => ({
    ...
    balance_forward: bill.balanceForward,
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Only clean up old JSON files, not PDFs (keep PDFs until new ones are downloaded)
    try {
    ...
    console.log(`Durham Water: Stored ${stored} new bills, updated ${updated} existing bills`);
    ...
    console.error("Durham Water API error:", error);
    return NextResponse.json(
      { error: "Failed to fetch Durham Water bills" },
      { status: 500 }
    );
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { bills } = body as { bills: (DurhamWaterBill & { matched_property_id?: string; stored_id?: string })[] };
    ...
    console.error("Failed to import Durham Water bills:", error);
    return NextResponse.json(

...
interface FetchResults {
  ...
  count: number;
  bills: XfinityBill[];
  requires_attention: XfinityBill[];
}

// Map document type string to enum
function mapDocType(docType: string): XfinityDocType {
  switch (docType) {
    case "bill":
      return XfinityDocType.BILL;
    case "disconnect_notice":
      return XfinityDocType.DISCONNECT_NOTICE;
    default:
      return XfinityDocType.UNKNOWN;
  }
}

// Store parsed bills in the database
async function storeParsedBills(
  bills: (XfinityBill & { matched_property_id: string | null })[]
  ...
  for (const bill of bills) {
    try {
      ...
      const existing = await prisma.xfinityParsedBill.findFirst({
        where: {
      ...
      if (existing) {
        await prisma.xfinityParsedBill.update({
          ...
          data: {
            ...
            internetCharges: bill.internet_charges || 0,
            taxesAndFees: bill.taxes_and_fees || 0,
        ...
        await prisma.xfinityParsedBill.create({
          data: {
            accountNumber: bill.account_number,
            serviceAddress: bill.service_address,
            documentType: mapDocType(bill.document_type),
...
async function loadStoredBills() {
  const bills = await prisma.xfinityParsedBill.findMany({
  ...
  return bills.map((bill) => ({
    ...
    internet_charges: bill.internetCharges,
    taxes_and_fees: bill.taxesAndFees,
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log(`Xfinity: Stored ${stored} new bills, updated ${updated} existing bills`);
    ...
    console.error("Xfinity API error:", error);
    return NextResponse.json(
      { error: "Failed to fetch Xfinity bills" },
      { status: 500 }
    );
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { bills } = body as { bills: (XfinityBill & { matched_property_id?: string; stored_id?: string })[] };
    ...
    console.error("Failed to import Xfinity bills:", error);
    return NextResponse.json(

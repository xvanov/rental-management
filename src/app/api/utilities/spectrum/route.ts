...
interface FetchResults {
  ...
  count: number;
  bills: SpectrumBill[];
  requires_attention: SpectrumBill[];
}

// Map document type string to enum
function mapDocType(docType: string): SpectrumDocType {
  switch (docType) {
    case "bill":
      return SpectrumDocType.BILL;
    case "disconnect_notice":
      return SpectrumDocType.DISCONNECT_NOTICE;
    default:
      return SpectrumDocType.UNKNOWN;
  }
}

// Store parsed bills in the database
async function storeParsedBills(
  bills: (SpectrumBill & { matched_property_id: string | null })[]
  ...
  for (const bill of bills) {
    try {
      ...
      const existing = await prisma.spectrumParsedBill.findFirst({
        where: {
      ...
      if (existing) {
        await prisma.spectrumParsedBill.update({
        ...
        await prisma.spectrumParsedBill.create({
          data: {
            accountNumber: bill.account_number,
            serviceAddress: bill.service_address,
            documentType: mapDocType(bill.document_type),
...
async function loadStoredBills() {
  const bills = await prisma.spectrumParsedBill.findMany({
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log(`Spectrum: Stored ${stored} new bills, updated ${updated} existing bills`);
    ...
    return NextResponse.json(
      { error: "Failed to fetch Spectrum bills" },
      { status: 500 }
    );
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { bills } = body as { bills: (SpectrumBill & { matched_property_id?: string; stored_id?: string })[] };
    ...
    console.error("Failed to import Spectrum bills:", error);
    return NextResponse.json(
